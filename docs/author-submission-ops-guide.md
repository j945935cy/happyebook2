# happyebook2 作者投稿 Apps Script 使用指南
...
- 將 `setShowProgressBar()` 改為 `setProgressBar()`
- 移除 Google Forms Apps Script 不支援的 `addFileUploadItem()`
- 將附件欄位改為雲端分享連結，避免建立表單時報錯
- 擴充投稿狀態，支援聯繫作者、簽約、排程上架、已上架
...
1. 使用表單填寫網址送出一筆測試資料
2. 打開回覆試算表，確認 `表單回覆` 已新增原始資料
3. 確認 `投稿管理` 已新增一筆資料，並包含：
...
請在 Apps Script 編輯器上方選擇函式名稱，再按「執行」。
...
- 這份版本的附件欄位採連結提交，不是 Google 表單原生檔案上傳
- 若需多人接收管理通知，請先執行 `setAdminNotificationEmails()`
